...
import co.candyhouse.sesame.open.device.CHSesame2Status
...
class WM2ScanSSIDListFG : BaseDeviceFG(R.layout.fg_wm2_scan_list), CHWifiModule2Delegate {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        L.d("hcia", "mDeviceModel.wm2LiveData.value:" + mDeviceModel.wm2LiveData.value!!.deviceStatus)
        refleshPage()

        swiperefresh.setOnRefreshListener { refleshPage() }

        mDeviceModel.wm2Delegates.put(mDeviceModel.wm2LiveData.value!!, object : CHWifiModule2Delegate {
                        ...
                        object : RecyclerView.ViewHolder(view), Binder<WifiRssi> {
                            ...
                            override fun bind(wifi_rssi: WifiRssi, pos: Int) {
                                ...
                                itemView.setOnClickListener {
                                    mDeviceModel.wm2LiveData.value?.setWifiSSID(wifi_rssi.ssid) {
                                        it.onSuccess {
                                            view.post {

                                                var defaultPWK = if (BuildConfig.DEBUG) "55667788" else null
                                                context?.inputTextAlert(getString(R.string.wm2_pwk_hint), wifi_rssi.ssid, defaultPWK) {
                                                    confirmButtonWithText("OK") { alert, name ->
                                                        mDeviceModel.wm2LiveData.value?.setWifiPassword(name) {}
                                                        mDeviceModel.wm2LiveData.value?.connectWifi {
                                                            it.onSuccess {
                                                                L.d("hcia", "連線成功it.data:" + it.data)
                                                                MainActivity.activity!!.runOnUiThread {
                                                                    Toast.makeText(MainActivity.activity!!, MainActivity.activity!!.getString(R.string.Wm2WifiOK), Toast.LENGTH_LONG).show()
                                                                }


//                                                                MainActivity.activity!!.selectorAlert("title", listOf("選擇Ａ","選擇Ｂ")){
//                                                                    L.d("hcia", "  it:" +   it)
//
//                                                                    if(it == 0){
//
...
//                                                                }?.show()
                                                            ...
                                                            it.onFailure {
                                                                MainActivity.activity!!.runOnUiThread {
                                                                    MainActivity.activity!!.alert(MainActivity.activity!!.getString(R.string.wm2_ap_setting_error),null){
                                                                        L.d("hcia", " this:" +  this)
                                                                    }?.show()
//                                                                    Toast.makeText(MainActivity.activity!!, "error:" + it, Toast.LENGTH_LONG).show()
                                                                }
                                                            }
                                                        }
                                                        dismiss()
    ...
    private fun refleshPage() {
        ...
        mDeviceModel.wm2LiveData.value!!.scanWifiSSID {
            it.onSuccess {
                L.d("hcia", "UI 掃描完畢")
